...
def voxelize(
    ...
    for sample in batch:
        ...
        original_normals.append(sample[4])
            ...
            if mode == "test":
                ...
                if "train" not in mode:
                    target_full = get_instance_masks(
                        list_coords=[torch.from_numpy(l) for l in original_coordinates],
                        list_labels=[torch.from_numpy(l) for l in original_labels],
                        task=task,
                        ignore_class_threshold=ignore_class_threshold,
                        filter_out_classes=filter_out_classes,
                        label_offset=label_offset,
...
def get_instance_masks(
    list_coords,
    list_labels,  #ins_label,sem_label， segment_id
    task,
    list_segments=None,
    ignore_class_threshold=100,
    filter_out_classes=[],
    ...
    for batch_id in range(len(list_labels)):
        ...
        for instance_id in instance_ids:
            ...
            label_id = tmp[0, 0]
            #tmp_coords = list_coords[batch_id][list_coords][batch_id][:,1]
            if (
            ...
            mask = list_labels[batch_id][:, 1] == instance_id
            masks.append(mask)

            coords_for_instance = list_coords[batch_id][mask].float()

            min_coords = coords_for_instance.min(dim=0)[0]
            max_coords = coords_for_instance.max(dim=0)[0]

            coords_for_instance_normalized = (coords_for_instance - min_coords) / (max_coords - min_coords + 1e-6)

            center = coords_for_instance_normalized.mean(dim=0)
        ...
        if task == "semantic_segmentation":
            ...
            if list_segments:
                ...
                target.append({"labels": label_ids, "masks": masks, "centers": centers})
        else:
            ...
            if list_segments:
                ...
                target.append({"labels": l, "masks": masks, "centers": centers})
    return target
