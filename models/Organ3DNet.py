...
from models.modules.sample_3deps import sample_3DEPS



class Organ3DNet(nn.Module):
    def __init__(
        self,
        config,
        hidden_dim,
        ...
        self.mask_features_heads = nn.ModuleList([
            conv(self.backbone.PLANES[i], self.mask_dim, kernel_size=1, stride=1, bias=True, D=3)
            for i in range(3, 8)  # 对应 PLANE[3], PLANE[4],PLANE[5], PLANES[6], PLANES[7]
        ])

        self.convtrs = nn.ModuleList([
            conv_tr(num_queries, num_queries, kernel_size=2, upsample_stride=2, dilation=1, bias=False, D=3)
            for _ in range(4)
        ])
        ...
        #self.out_bbox = nn.Sequential(nn.Linear(hidden_dim, hidden_dim), nn.ReLU(), nn.Linear(hidden_dim, 3))
    ...
    def forward(
        self, x, point2segment=None, raw_coordinates=None, is_edge=None, is_eval=False
    ):
        ...
        features = [head(pcd_features if i == 0 else aux[-(i+1)]) for i, head in enumerate(self.mask_features_heads[::-1])]
        sampled_coords = None
        ...
        if self.non_parametric_queries:
            ...
            fps_idx = [
                sample_3DEPS(
                    x.decomposed_coordinates[i][None, ...].float(),
                    is_edge.decomposed_features[i][None, ...],
                    self.num_queries,
                )
                for i in range(len(x.decomposed_coordinates))
            ]
        ...
        for decoder_counter in range(self.num_decoders):
            ...
            for i, hlevel in enumerate(self.hlevels):
                ...
                decomposed_attn = attn_mask.decomposed_features  ## attn_mask中合理的点（sigmoid()>0.5) 设置为false
                ...
                rand_idx = []
                mask_idx = []  #这里的rand_idx和mask_idx是用来筛选当pad或sample两种情况下的有效点的
                ...
                # -----------------------------self_attention前进行edgeconv
                # output = self.edge_conv_queries(output, k=4, d=1)
    ...
    def mask_module(
        ...
        Args:
            query_feat: b,k,128
            mask_features: [(N1,128),(N2,128)...] N1>N2>...
        Returns:
        ...
        output_mask = [
            torch.cat([feature.decomposed_features[i] @ mask_embed[i].T for i in range(feature.C[-1, 0] + 1)])
            for feature in mask_features
        ]  #[(N1,K),(N2,K)...]
        outputs_mask = [
            me.SparseTensor(
                features=output_mask[-(i + 1)],
                coordinate_manager=mask_features[-(i + 1)].coordinate_manager,
                coordinate_map_key=mask_features[-(i + 1)].coordinate_map_key
            ) for i in range(5)
        ] #[(N5,k),(N4,k)...]  N5<N4<...
        for i in range(4):
            current_layer = outputs_mask[i]
            next_layer = outputs_mask[i + 1]
            upsampled_current_layer = self.convtrs[i](current_layer)
            combined_layer = upsampled_current_layer + next_layer
            outputs_mask[i + 1] = combined_layer
        final_output = outputs_mask[4]

        if ret_attn_mask:
            attn_mask = final_output
            ...
            )  # attn_mask中合理的点（sigmoid()>0.5) 设置为false的，当 sigmoid 函数的输出值小于 0.5 时，布尔掩码的对应位置为 True


            return (
                    outputs_class,
                    final_output.decomposed_features,
                    attn_mask,
                )
        return outputs_class, final_output.decomposed_features
