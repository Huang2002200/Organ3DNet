...
class Res16UNetBase(ResNetBase):  #BLOCK = BasicBlock
    ...
    def forward(self, x):  #x:(9473, 3)
        feature_maps = []

        out = self.conv0p1s1(x)  #out:(9473, 32)
        out = self.bn0(out)
        out_p1 = self.relu(out)   #out_p1:(9473, 32)

        out = self.conv1p1s2(out_p1)  #out:(3641, 32)
        out = self.bn1(out)
        out = self.relu(out)
        out_b1p2 = self.block1(out)   #out_b1p2:(3641, 32)

        out = self.conv2p2s2(out_b1p2)  #out:(1009,32)
        out = self.bn2(out)
        out = self.relu(out)
        out_b2p4 = self.block2(out)   #out_b2p4:(1009,64)

        out = self.conv3p4s2(out_b2p4)  #out:(280,64)
        out = self.bn3(out)
        out = self.relu(out)
        out_b3p8 = self.block3(out)   #out_b3p8:(280,128)

        # pixel_dist=16
        out = self.conv4p8s2(out_b3p8)  #(93,128)
        out = self.bn4(out)
        out = self.relu(out)
        out = self.block4(out)  #(93,256)

        feature_maps.append(out)

        # pixel_dist=8
        out = self.convtr4p16s2(out)  #(280,256)
        out = self.bntr4(out)
        out = self.relu(out)

        out = me.cat(out, out_b3p8)  #(280,384)
        out = self.block5(out)  #(280,256)

        feature_maps.append(out)

        # pixel_dist=4
        out = self.convtr5p8s2(out)  #(1009,128)
        out = self.bntr5(out)
        out = self.relu(out)

        out = me.cat(out, out_b2p4)  #(1009,192)
        out = self.block6(out)  #(1009,128)

        feature_maps.append(out)

        # pixel_dist=2
        out = self.convtr6p4s2(out)  #(3641,96)
        out = self.bntr6(out)
        out = self.relu(out)

        out = me.cat(out, out_b1p2)  #(3641,128)
        out = self.block7(out)  #(3641,96)

        feature_maps.append(out)

        # pixel_dist=1
        out = self.convtr7p2s2(out)  #(9473,96)
        out = self.bntr7(out)
        out = self.relu(out)

        out = me.cat(out, out_p1) #(9473,128)
        out = self.block8(out)  #(9473,96)
...
class Res16UNet34(Res16UNetBase):
    BLOCK = BasicBlock
    LAYERS = (2, 3, 4, 6, 2, 2, 2, 2)  #PLANES = (32, 64, 128, 256, 256, 128, 96, 96)
