...
class InstanceSegmentation(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        if self.config.data.add_raw_coordinates:
            ...
            is_edge = data.features[:, -4:-3]
            data.features = data.features[:, :-4]    #features=color等+raw_coordinates,没有坐标，考虑加上坐标？
    ...
    def eval_step(self, batch, batch_idx):
        ...
        if self.config.general.save_visualizations:
            ...
            )  ##将backbone_features输出特征从高维降为3维，并缩放到0-255
    ...
    def get_mask_and_scores(
        ...
        heatmap = torch.where(heatmap < 0.5, torch.tensor(0.0, device=heatmap.device), heatmap)
    ...
    def eval_instance_step(
        ...
        for bid in range(len(prediction[self.decoder_id]["pred_masks"])):
            ...
            sorted_masks = masks[:, sort_scores_index]    #把掩码按得分排序
    ...
    def eval_instance_epoch_end(self):
        ...
        print(f"validation_dataset.data_dir: {self.validation_dataset.data_dir[0]}")

        gt_data_path = f"{self.validation_dataset.data_dir[0]}/instance_gt/Area_{self.config.general.area}"
        print(f"gt_data_path: {gt_data_path}")
        ...
        try:
            if self.validation_dataset.dataset_name == "5plant":
                new_preds = {}
                ...
                outputs = evaluate(
                    new_preds, gt_data_path, pred_path, dataset="5plant"
                )
                ap_results[f"{log_prefix}_mean_ins_precision"] = outputs[0]
                ap_results[f"{log_prefix}_mean_ins_recall"] = outputs[1]
                ap_results[f"{log_prefix}_mean_mucov"] = outputs[2]
                ap_results[f"{log_prefix}_mean_mwcov"] = outputs[3]
                ap_results[f"{log_prefix}_mean_sem_precision"] = outputs[4]
                ap_results[f"{log_prefix}_mean_sem_recall"] = outputs[5]
                ap_results[f"{log_prefix}_mean_iou"] = outputs[7]
                ap_results[f"{log_prefix}_mean_f1_score"] = outputs[6]
                print("mean_ins_precision", outputs[0])
                print("mean_ins_recall", outputs[1])
                print("mean_mucov", outputs[2])
                print("mean_mwcov", outputs[3])
                print("mean_sem_precision", outputs[4])
                print("mean_sem_recall", outputs[5])
                print("mean_iou", outputs[7])
                print("mean_f1_score", outputs[6])
